...
if sys.argv[argshift] == '-m':
    with open(sys.argv[argshift + 1], 'r') as mapfile:
        for line in mapfile:
            match = re.match('[0-9a-f]{8} [0-9a-f]{8} ([0-9a-f]{8}) [0-9] ([^ 0-9.][^ ]*)', line)
            if match:
                addr = int(match.group(1), 16)
                name = format(match.group(2).rstrip("\n"))
                labels.add(addr)
                
                if not name in blacklistedLabelNames:
                    if name in labelNames.values():
                        i = 0
                        
                        while True:
                            testname = "%s_%i" % (name, i)
                            
                            if not testname in labelNames.values():
                                name = testname
                                break
...
blacklistedInsns = {
    ...
    PPC_INS_VMRGHB, PPC_INS_XXSPLTW, PPC_INS_XVNMSUBMDP, PPC_INS_XSMSUBADP,
    PPC_INS_XSMADDMDP, PPC_INS_XVCMPGEDP, PPC_INS_XVNMSUBADP, PPC_INS_VPKUHUM,
    PPC_INS_XSCMPODP, PPC_INS_XVCMPGTDP, PPC_INS_XVCMPEQDP, PPC_INS_VMADDFP,
    PPC_INS_VADDUDM, PPC_INS_XSMADDADP, PPC_INS_XSNMSUBADP, PPC_INS_XVMSUBMSP,
    PPC_INS_XVNMADDMSP, PPC_INS_XSCMPUDP, PPC_INS_VADDUBM, PPC_INS_XVNMADDMDP,
    PPC_INS_XVMSUBADP, PPC_INS_XVNMSUBMSP, PPC_INS_XVNMSUBASP, PPC_INS_XVMADDMDP,
    PPC_INS_XVMADDADP, PPC_INS_XVNMADDADP,
