 ...
 *         success.
 */
typedef RtFileSystemError (*RtFileOpenFunc)(RtFileSystem* fs, RtFile* file, const RwChar* filename,
                                            RwUInt32 flags);
 ...
 */
typedef void (*RtFSFileOpCallBack)(void* file, RwUInt32 size, RwUInt32 status,
...
extern RtFileSystemError _rtFSInit(RtFileSystem* fs, RwInt32 maxNbFiles,
                                   RwChar fsName[RTFS_MAX_NAME_LENGTH], RwChar* deviceName);

extern const RwChar* _rtGetFileNameFromDeviceName(const RwChar* fileName);

extern RwChar* _rtFSBuildFileName(RwChar* buffer, RwInt32 bufferLength, const RwChar* driveName,
                                  const RwChar* filePath, RwChar separator);

extern RwBool _rtFSFileExists(RtFileSystem* fs, const RwChar* fileName);

extern RwBool _rtFSIsEOF(RtFile* file);

extern RwBool RtFileSystemSetCallBack(RtFileSystem* fs, RtFileSystemCallBackCode cbCode,
                                      RtFileSystemCallBack CallBack);

extern RtFileSystemCallBack RtFileSystemGetCallBack(RtFileSystem* fs,
                                                    RtFileSystemCallBackCode cbCode);

extern void _rtFileSystemTriggerCallBack(RtFileSystem* fs, RtFileSystemCallBackCode cbCode);

extern RtFileSystemError RtFileSystemSetDeviceName(RtFileSystem* fs, RwChar* deviceName);

extern RwChar* RtFileSystemGetDeviceName(RtFileSystem* fs);

extern void RtFileSystemDefaultToAsync(RtFileSystem* fs, RwBool isAsync);

extern RwBool RtFileSystemIsDefaultedToAsync(RtFileSystem* fs);

extern RtFileSystemStatus RtFileSystemGetStatus(RtFileSystem* fs);

extern void RtFileSystemSetOustandingOpCallback(RtFile* file, RtFSFileOpCallBack CallBack,
                                                void* CallBackData);

extern void rtFSTriggerFileOustandingOpCallback(RtFile* file);

#if (defined(WIN32) || defined(MACOS))
#define _GetPosition(_file) (((RtFile*)_file)->position.noSupportValue.low)
#define _GetSize(_file) (((RtFile*)_file)->length.noSupportValue.low)
#else
#define _GetPosition(_file) (((RtFile*)_file)->position.supportValue)
#define _GetSize(_file) (((RtFile*)_file)->length.supportValue)
#endif
...
extern void* RwFopen(const RwChar* name, const RwChar* access);

extern RwInt32 RwFclose(void* fptr);

extern size_t RwFread(void* addr, size_t size, size_t count, void* fptr);

extern size_t RwFwrite(const void* addr, size_t size, size_t count, void* fptr);

extern RwInt32 RwFseek(void* fptr, long offset, RwInt32 origin);

extern RwChar* RwFgets(RwChar* buffer, RwInt32 maxLen, void* fptr);

extern RwInt32 RwFputs(const RwChar* buffer, void* fptr);

extern RwInt32 RwFeof(void* fptr);

extern RwBool RwFexist(const RwChar* name);

extern RwInt32 RwFtell(void* fptr);

extern RwBool RwFAsyncCancel(RtFile* fptr);

extern RwBool RtFSManagerOpen(RwInt32 maxNbFS);

extern void RtFSManagerClose(void);

extern RwBool RtFSManagerSetCallBack(RtFSManagerCallBackCode cbCode, RtFSManagerCallBack CallBack);

extern RwBool RtFSManagerRegister(RtFileSystem* fs);

extern void RtFSManagerUnregister(RtFileSystem* fs);

extern RtFileSystem* RtFSManagerGetFileSystemFromName(RwChar* fsName);

extern RtFile* RtFSManagerFOpen(const RwChar* filename, RtFileAccessFlag access,
                                RtFSFileOpCallBack CallBack, void* CallBackData);

extern RtFileSystem* RtFSManagerGetFileSystemFromFileName(const RwChar* fileName);

extern void RtFSManagerSetDefaultFileSystem(RtFileSystem* fs);

extern RtFileSystem* RtFSManagerGetDefaultFileSystem(void);

extern void RtFSManagerSyncAllFilesOnFileSystem(RtFileSystem* fs);

extern void RtFSManagerSyncAllFiles(void);

extern RtFSManagerError RtFSManagerGetLastError(void);

extern RwBool _rtFSManagerIsNewFSRegistrable(void);

extern RwBool _rtFSManagerIsFSNameInUse(RwChar* fsName);

extern RtFileSystem* _rtGetFileSystemFromDeviceName(const RwChar* deviceName);

extern RtFile* _rtFSManagerFOpenOnFS(RtFileSystem* fs, const RwChar* name, RtFileAccessFlag access,
                                     RtFSFileOpCallBack CallBack, void* CallBackData);
